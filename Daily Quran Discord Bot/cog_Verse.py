...
class VerseCog(commands.Cog):
    def __init__(self, bot):
        ...
        console_log(f"Loaded {len(self.verses)} verses from quran.txt")
    ...
    def load_verses(self):
        ...
        try:
            ...
            error_msg = "ERROR: quran.txt file not found!"
            console_log(error_msg)
            log_error(error_msg)
            return []
        except Exception as e:
            error_msg = f"Error loading quran.txt: {str(e)}"
            console_log(error_msg)
            log_error(error_msg)
            return []
    ...
    async def send_verse_to_server(self, server_id, was_down=False, missed_count=0):
        try:
            ...
            if not bot_member or not channel.permissions_for(bot_member).send_messages:
                log_error(f"Missing permissions in server {server_id}, channel {channel_id}")
                conn.close()
                return
            ...
            if full_verse_text:
                ...
                console_log(f"Sent verse {current_verse} to server {server_id}, next UTC: {next_send_utc.strftime('%Y-%m-%d %H:%M:%S')}")
            else:
                log_error(f"Verse not found: {current_verse} for server {server_id}")
            
            conn.close()
            
        except discord.errors.Forbidden:
            error_msg = f"Missing permissions in server {server_id}"
            log_error(error_msg)
        except Exception as e:
            error_msg = f"Error sending verse to server {server_id}: {str(e)}"
            log_error(error_msg)
